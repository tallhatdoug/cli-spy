...
matrix:
        ...
        for t in x86_64 aarch64 i386; do \
            zig cc -target $$t-linux-musl -O2 -static -Ivendor \
                -o dist/cli-spy-$$t cli-spy.c seal.c vendor/tweetnacl.c -lm -lpt   hread; \
        done
        zig cc -target armv7l-linux-musleabihf -O2 -static -Ivendor \
            -o dist/cli-spy-armv7l cli-spy.c seal.c vendor/tweetnacl.c -lm -lpth   read

clean:
        rm -f cli-spy $(OBJ)

.PHONY: static matrix clean
